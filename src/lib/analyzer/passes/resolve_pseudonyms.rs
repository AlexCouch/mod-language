...
fn resolve_pseudonym (analyzer: &mut Analyzer, pseudonyms: &mut Vec<Pseudonym>, pseudonym: Pseudonym) -> Option<ContextKey> {
  ...
  let mut resolved_key = pseudonym.relative_to;
  
  for ident in pseudonym.chain.iter() {
    ...
    if let ContextItem::Namespace(namespace) = base {
      ...
      resolved_key = if !pseudonym.absolute && resolved_key == pseudonym.relative_to {
        if let Some(local) = namespace.local_bindings.get_entry(ident) {
          ...
          analyzer.error(pseudonym.origin, format!("Namespace `{}` does not have access to an item named `{}`", base_name, ident));
          return None
        }
        ...
        analyzer.error(pseudonym.origin, format!("Namespace `{}` does not export an item named `{}`", base_name, ident));
        return None
      };
    } else {
      analyzer.error(pseudonym.origin, format!("{} is not a Namespace and has no exports", ident));
      return None
    }
  }
